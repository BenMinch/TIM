...
def getNameGivenTAXIDandRank(TAXID, RANK):
    ...
    taxid_for_targetedRank = None

    for taxid, rank in dicoRank.items():
        if rank == RANK:
            taxid_for_targetedRank = taxid
            break

    if taxid_for_targetedRank:
        taxid_translator = ncbi.get_taxid_translator([taxid_for_targetedRank])
        name_for_targetedRank=list(taxid_translator.values())[0]
    else:
        name_for_targetedRank = "NA"
...
def getCoPerEukGrp(FILE):
	...
	for l in f:
		...
		if LML in ncbiNames:
			LML_taxid=ncbi.get_name_translator([LML]).values()
			value_list=list(LML_taxid)
			LML_taxid=value_list[0][0]
...
for node in t.iter_descendants("preorder"):
	...
	for lev in setLevel:
		...
		if lev=="NA":
			...
			taxid=ncbi.get_name_translator([lev]).values()
			taxid=list(taxid)[0]
			rank=ncbi.get_rank(taxid).values()
			rank=list(rank)[0]
